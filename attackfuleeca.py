...
def pw(x,p =65521, beta =0.021240595234768,Z = 94.1628598983449): #probability of x occuring
...
def NumberInTLS(x, halfn,p =65521, beta =0.021240595234768,Z = 94.1628598983449,tau=0.25):
    '''Given an element in the field,how many of it will be in the TLS'''
    a=pw(x,p,beta,Z)*halfn
    if a >= 1:
        b=floor(a)
    else:
        if a >= tau:
...
def MakeTLS(halfn,p =65521, beta =0.021240595234768,Z = 94.1628598983449,tau=0.25):
    '''Given the value of halfn, generate the typical Lee Set'''
    TLS=[]
    for i in range(p):
        TLS= TLS + [i for j in range(NumberInTLS(i,halfn,p,beta,Z,tau))]
    if len(TLS)<halfn:
...
def get_T(T_csv):
    ...
    return(T)        
def T_to_dat(level=0, T ,filename = 'toyexample'):
